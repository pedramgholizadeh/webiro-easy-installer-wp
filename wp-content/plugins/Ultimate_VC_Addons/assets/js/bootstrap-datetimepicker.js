 ...
 */
( function ( $ ) {
	const smartPhone = window.orientation != undefined;
	const DateTimePicker = function ( element, options ) {
		this.id = dpgId++;
		this.init( element, options );
	};
	const dateToDate = function ( dt ) {
		if ( typeof dt === 'string' ) {
			return new Date( dt );
		}
		return dt;
	...
	DateTimePicker.prototype = {
		...
		init( element, options ) {
			...
			this.minViewMode =
				options.minViewMode ||
				this.$element.data( 'date-minviewmode' ) ||
				0;
			...
			this.setEndDate(
				options.endDate || this.$element.data( 'date-enddate' )
			);
			this.fillDow();
			this.fillMonths();
			this.fillHours();
		...
		hide() {
			const collapse = this.widget.find( '.collapse' );
			for ( let i = 0; i < collapse.length; i++ ) {
				const collapseData = collapse.eq( i ).data( 'collapse' );
		...
		fillDate() {
			...
			const currentYear = this._date.getUTCFullYear();
			const months = this.widget
				.find( '.datepicker-months' )
				.find( 'th:eq(1)' )
				.text( year )
				.end()
		...
		fillMinutes() {
			...
			for ( let i = 0; i < 5; i++ ) {
				...
				for ( let j = 0; j < 4; j += 1 ) {
					...
					html +=
						'<td class="minute">' + padLeft( c, 2, '0' ) + '</td>';
					current += 3;
				}
				html += '</tr>';
			}
		...
		fillSeconds() {
			...
			for ( let i = 0; i < 5; i++ ) {
				...
				for ( let j = 0; j < 4; j += 1 ) {
					const c = current.toString();
					html +=
						'<td class="second">' + padLeft( c, 2, '0' ) + '</td>';
					current += 3;
				}
				html += '</tr>';
			}
		...
		fillTime() {
			...
			const table = timeComponents.closest( 'table' );
			const is12HourFormat = this.options.pick12HourFormat;
			let hour = this._date.getUTCHours();
			let period = 'AM';
			if ( is12HourFormat ) {
			...
			const minute = padLeft(
				this._date.getUTCMinutes().toString(),
				2,
				'0'
			);
			const second = padLeft(
				this._date.getUTCSeconds().toString(),
				2,
				'0'
			);
		...
		actions: {
			...
			selectHour( e ) {
				const tgt = $( e.target );
				let value = parseInt( tgt.text(), 10 );
				if ( this.options.pick12HourFormat ) {
					const current = this._date.getUTCHours();
					if ( current >= 12 ) {
			...
			selectMinute( e ) {
				const tgt = $( e.target );
				const value = parseInt( tgt.text(), 10 );
		...
		doAction( e ) {
			...
			const rv = this.actions[ action ].apply( this, arguments );
			this.set();
			this.fillTime();
			this.notifyChange();
			return rv;
		...
		keypress( e ) {
			...
			let val = input.val() || '';
			val += c;
			let mask = this._mask[ this._maskPos ];
			if ( ! mask ) {
				return false;
			}
			...
			if ( ! mask.pattern.test( val.slice( mask.start ) ) ) {
				...
				if ( mask.end != val.length ) {
					input.val( val );
					return false;
				} else if ( ! mask.pattern.test( val.slice( mask.start ) ) ) {
		...
		_compileFormat() {
			...
			while ( ( match = formatComponent.exec( str ) ) ) {
				...
				if ( component in dateFormatComponents ) {
					i++;
					propertiesByIndex[ i ] =
						dateFormatComponents[ component ].property;
					components.push(
						'\\s*' +
			...
			if ( this.pickDate && this.pickTime ) {
				this.widget.on(
					...
					function ( e ) {
						...
						const expanded = $parent.find( '.collapse.in' );
						const closed = $parent.find( '.collapse:not(.in)' );
	...
	const keys = [];
	for ( const k in dateFormatComponents ) keys.push( k );
	...
	function getTemplate(
		...
		if ( pickDate && pickTime ) {
			return (
				...
				'</div>' +
				'</li>' +
				'<li class="picker-switch accordion-toggle"><a><i class="icon-stopwatch"></i></a></li>' +
				'<li' +
	...
	TPGlobal.getTemplate = function ( is12Hours, showSeconds ) {
		return (
			...
			'>' +
			'<tr>' +
			'<td><a href="#" class="btn" data-action="incrementHours"><i class="icon-arrow-up"></i></a></td>' +
			'<td class="separator"></td>' +
			'<td><a href="#" class="btn" data-action="incrementMinutes"><i class="icon-arrow-up"></i></a></td>' +
			( showSeconds
				? '<td class="separator"></td>' +
				  '<td><a href="#" class="btn" data-action="incrementSeconds"><i class="icon-arrow-up"></i></a></td>'
				: '' ) +
			...
			'</tr>' +
			'<tr>' +
			'<td><a href="#" class="btn" data-action="decrementHours"><i class="icon-arrow-down"></i></a></td>' +
			'<td class="separator"></td>' +
			'<td><a href="#" class="btn" data-action="decrementMinutes"><i class="icon-arrow-down"></i></a></td>' +
			( showSeconds
				? '<td class="separator"></td>' +
				  '<td><a href="#" class="btn" data-action="decrementSeconds"><i class="icon-arrow-down"></i></a></td>'
				: '' ) +
			( is12Hours ? '<td class="separator"></td>' : '' ) +
			'</tr>' +
			'</table>' +
