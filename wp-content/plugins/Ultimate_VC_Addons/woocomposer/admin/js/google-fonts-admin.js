...
$gf( document ).ready( function () {
	$gf( 'body' ).on( 'click', '#refresh-google-fonts', function () {
		...
		$gf.post( ajaxurl, data, function ( response ) {
			const object = jQuery.parseJSON( response );
		...
		if ( ! button.hasClass( 'font-added' ) ) {
			...
			button
				...
				.each( function ( iv, variant ) {
					$v = $gf( variant );
					const temp_arr = {};
					const variant_value = $v
						.find( '.font-variant-inputs' )
						.val();
			...
			$gf.post( ajaxurl, data, function ( response ) {
				...
				if ( variants_array.length > 0 ) var xclass = 'have-variants';
				else var xclass = '';
				let gshtml =
				...
				if ( variants_array.length > 0 ) {
					...
					$gf.each( variants_array, function ( index, variant ) {
						...
						let weight = 'normal';
						if (
							( weight = variant.variant_value.match( /\d+/ ) )
						) {
	...
	$gf( 'body' ).on( 'click', '.font-delete', function () {
		...
		$gf( 'body' ).append(
			'<div class="google-font-confirmation"><div class="google-font-confirmation-header"><h3>Are you sure?</h3></div><div class="google-font-message"><input type="button" id="" data-gfont_name="' +
				font_name +
				'" class="google-font-message-buttons google-font-message-delete gfont-buttons" value="Yes, Please delete font"/><input type="button" id="" class="google-font-message-buttons google-font-message-no-delete gfont-buttons" value="No, Keep it"/></div><a href="javacript:void(0)" class="gfont-anchor-buttons gfont-buttons google-font-message-no-delete"><i class="dashicons dashicons-no-alt"></i></a></div>'
		);
...
function get_google_fonts() {
	...
	$gf.post( ajaxurl, data, function ( response ) {
		...
		if ( font_count == 0 && is_search == 'false' ) {
			var ghtml =
				'<div class="gfont">It seems you don\'t have any Google Fonts yet. But you can download them now with <a href="javascript:void(0)" id="refresh-google-fonts">just a click.</a> <span class="spinner"></span></div>';
		...
		$list_wrapper.attr(
			'data-gstart',
			parseInt( gstart ) + parseInt( gfetch )
		);
	} );
}
...
function set_wrap_height( fx ) {
	const win_height = $gf( window ).height();
	if ( fx == 'notfixed' ) {
		const swrap_top = $gf( '#fonts-selected-wrapper' ).offset().top;
