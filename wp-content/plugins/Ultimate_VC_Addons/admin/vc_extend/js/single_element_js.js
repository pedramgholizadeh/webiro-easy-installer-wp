( function ( $ ) {
	...
	window.UltimateSubTabView = window.VcColumnView.extend( {
		...
		changeShortcodeParams( model ) {
			...
			window.UltimateSubTabView.__super__.changeShortcodeParams.call(
				this,
				model
			);
			...
			if (
				_.isObject( params ) &&
				_.isString( params.title ) &&
				_.isString( params.tab_id )
			) {
				$( '.ui-tabs-nav [href="#tab-' + params.tab_id + '"]' ).text(
					params.title
				);
			}
		...
		deleteShortcode( e ) {
			...
			const answer = confirm(
					window.i18nLocale.press_ok_to_delete_section
				),
				parent_id = this.model.get( 'parent_id' );
			if ( answer !== true ) return false;
			this.model.destroy();
			if ( ! vc.shortcodes.where( { parent_id } ).length ) {
			...
			$( '[href=#tab-' + params.tab_id + ']' )
				.parent()
				.remove();
			const tab_length = this.$tabs.find(
				'.ui-tabs-nav li:not(.add_tab_block)'
			).length;
			if ( tab_length > 0 ) {
			...
			if ( model.get( 'shortcode' ) === 'single_tab' )
				_.extend( new_params, {
					tab_id:
						+new Date() +
						'-' +
						this.$tabs.find( '[data-element_type=single_tab]' )
							.length +
						'-' +
						Math.floor( Math.random() * 11 ),
				} );
			const model_clone = Shortcodes.create( {
