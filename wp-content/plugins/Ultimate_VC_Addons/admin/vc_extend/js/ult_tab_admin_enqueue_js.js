( function ( $ ) {
	...
	if ( typeof vc === 'undefined' || typeof vc.shortcode_view === 'undefined' )
		return false;
	const Shortcodes = vc.shortcodes;
	window.UltimateTabView = vc.shortcode_view.extend( {
		...
		createAddTabButton() {
			const new_tab_button_id =
				+new Date() + '-' + Math.floor( Math.random() * 11 );
			this.$tabs.append(
				'<div id="new-tab-' +
					new_tab_button_id +
					'" class="new_element_button"></div>'
			);
			this.$add_button = $(
				'<li class="add_tab_block"><a href="#new-tab-' +
					new_tab_button_id +
					'" class="add_tab" title="' +
					window.i18nLocale.add_tab +
					'"></a></li>'
			).appendTo( this.$tabs.find( '.tabs_controls' ) );
		...
		addTab( e ) {
			...
			const tab_title = window.i18nLocale.tab,
				tabs_count = this.$tabs.find( '[data-element_type=single_tab]' )
					.length,
				tab_id =
					+new Date() +
					'-' +
		...
		changedContent( view ) {
				...
				this.$tabs.find( '.ui-tabs-nav' ).sortable( {
					axis:
						this.$tabs
							...
							.data( 'element_type' ) == 'test_element'
							? 'y'
							: 'x',
			...
			if ( view.model.get( 'cloned' ) === true ) {
				const cloned_from = view.model.get( 'cloned_from' ),
					$tab_controls = $(
				...
				this.$tabs.tabs(
					...
					'active',
					this.new_tab_adding
						? $( '.ui-tabs-nav li', this.$content ).length - 2
						: 0
				);
			}
		...
		cloneModel( model, parent_id, save_order ) {
			let shortcodes_to_resort = [],
				new_order =
					_.isBoolean( save_order ) && save_order === true
						? model.get( 'order' )
						: parseFloat( model.get( 'order' ) ) + vc.clone_index,
				model_clone,
				new_params = _.extend( {}, model.get( 'params' ) );
			if ( model.get( 'shortcode' ) === 'single_tab' )
				_.extend( new_params, {
					tab_id:
						+new Date() +
						'-' +
						this.$tabs.find( '[data-element-type=single_tab]' )
							.length +
						'-' +
						Math.floor( Math.random() * 11 ),
				} );
			model_clone = Shortcodes.create( {
				...
				order: new_order,
				cloned:
					model.get( 'shortcode' ) === 'single_tab' ? false : true,
				cloned_from: model.toJSON(),
				params: new_params,
			} );
			_.each(
				Shortcodes.where( { parent_id: model.id } ),
				function ( shortcode ) {
...
jQuery( document ).ready( function () {
	jQuery( 'body' ).on( 'change', '.disp_icon', function () {
		const op = jQuery( this ).data( 'option' );
		if ( op == 'Disables' ) {
			jQuery( this )
				.parent()
				.parent()
				.parent()
				.find( '.ult_tabicon_notice' )
				.css( { display: 'none' } );
		} else {
			jQuery( this )
				.parent()
