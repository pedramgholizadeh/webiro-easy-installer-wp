...
( function ( $, window, undefined ) {
	function update_visibility( t ) {
		const status =
			t.find( '.ultimate-border-style-selector option:selected' ).val() ||
			'none'; // set data status
		if ( status === 'none' ) {
			t.find(
				'.ultimate-four-input-section, .ultimate-border-radius-block, .ultimate-colorpicker-section'
			).hide();
		} else {
			t.find(
				'.ultimate-four-input-section, .ultimate-border-radius-block, .ultimate-colorpicker-section'
	...
	function init( t ) {
		//  Hide {all}
		t.find( '.ultb-width-all, .ultb-radius-all' ).attr(
			'data-status',
			'hide-all'
		);
		t.find( '.ultb-width-single, .ultb-radius-single' ).hide();
		t.find(
			'.ultb-width-section .ult-expand, .ultb-radius-section .ult-expand'
		).addClass( 'ult-collapse' );
		...
		t.find( '.ultb-width-section .ult-expand' ).click( function ( event ) {
			...
			if ( status === 'hide-me' ) {
				t.find( '.ultb-width-section .ult-expand' ).addClass(
					'ult-collapse'
				);
				t.find( '.ultb-width-all' ).attr( 'data-status', 'hide-all' );
			} else {
				t.find( '.ultb-width-section .ult-expand' ).removeClass(
					'ult-collapse'
				);
				t.find( '.ultb-width-all' ).attr( 'data-status', 'hide-me' );
			}
		...
		t.find( '.ultb-radius-section .ult-expand' ).click( function ( event ) {
			...
			if ( status === 'hide-me' ) {
				t.find( '.ultb-radius-section .ult-expand' ).addClass(
					'ult-collapse'
				);
				t.find( '.ultb-radius-all' ).attr( 'data-status', 'hide-all' );
			} else {
				t.find( '.ultb-radius-section .ult-expand' ).removeClass(
					'ult-collapse'
				);
				t.find( '.ultb-radius-all' ).attr( 'data-status', 'hide-me' );
			}
		...
		//  unit change
		t.find( '.ult-unit-border-width, .ult-unit-border-radius' ).change(
			function ( event ) {
				set_hidden_with_border_style( t );
			}
		);
	...
	function get_hidden_with_border_style( t ) {
		const l = t.find( '.ultimate-border-style-selector' ).length;
		const mv = t.find( '.ultimate-border-value' ).val();

		if ( mv != '' ) {
			if ( l ) {
				const vals = mv.split( '|' );
				// set border style
				const splitval = vals[ 0 ].split( ':' );
				const bstyle = splitval[ 1 ].split( ';' );
				t.find( '.ultimate-border-style-selector' ).val( bstyle[ 0 ] );
				t.find( '.ultimate-border-style-selector' ).trigger(
					'chosen:updated'
				);

				// Disable inputs if border-style - {none}
				if (
					typeof bstyle[ 0 ] !== 'undefined' &&
					bstyle[ 0 ] != null
				) {
					if ( bstyle[ 0 ] === 'none' ) {
						update_visibility( t );
			...
			$.each( bw, function ( i, vl ) {
				if ( vl != '' ) {
					t.find( '.ultimate-border-inputs' ).each( function (
						input_index,
						elem
					) {
						...
						if (
							...
							switch ( splitval[ 0 ] ) {
								case 'border-width':
									...
									t.find( '.ultb-width-all' ).attr(
										'data-status',
										'hide-all'
									);
									t.find( '.ultb-width-single' ).hide();
									t.find(
										'.ultb-width-section .ult-expand'
									).addClass( 'ult-collapse' );
									break;
								...
								case 'border-left-width':
									...
									t.find(
										'.ultb-width-section .ult-expand'
									).removeClass( 'ult-collapse' );
									break;

								case 'border-radius':
									t.find( '.ultb-radius-all' ).show();
									t.find( '.ultb-radius-all' ).attr(
										'data-status',
										'hide-all'
									);
									t.find( '.ultb-radius-single' ).hide();
									t.find(
										'.ultb-radius-section .ult-expand'
									).addClass( 'ult-collapse' );
									break;
							...
							switch ( splitval[ 0 ] ) {
								...
								case 'border-left-width':
									...
									t.find( '.ult-unit-border-width' ).val(
										unit
									); // set border select unit
								...
								case 'border-bottom-left-radius':
									...
									t.find( '.ult-unit-border-radius' ).val(
										unit
									); // set border select unit
									if ( dataid == splitval[ 0 ] ) {
										mval = splitval[ 1 ].split( unit );
			...
			// set color
			const splitcols = mv.split( '|' );
			if (
				typeof splitcols[ 2 ] !== 'undefined' ||
				splitcols[ 2 ] != null
			) {
				const sp = splitcols[ 2 ].split( ':' );
				const nd = sp[ 1 ].split( ';' );
				const did = t.find( '.ultimate-colorpicker' ).attr( 'data-id' );
				if ( sp[ 0 ] == did ) {
					if ( nd[ 0 ] !== 'transparent' ) {
						t.find( '.ultimate-colorpicker' )
							.val( nd[ 0 ] )
							.trigger( 'change' );
						//t.find("a.wp-color-result").css({"background-color": nd[0]});
						//   set alpha value
						const picker = $.cs_ParseColorValue( nd[ 0 ] );
	...
	$(
		'.ultimate-border-input, .ultimate-border-style-selector, .ultimate-colorpicker'
	).on( 'change', function ( e ) {
		const t = $( this ).closest( '.ultimate-border' );
		const v = t.find( '.ultimate-border-value' ).val();
	...
	function set_hidden_with_border_style( t, hxcolor ) {
		...
		if ( l ) {
			const sv = t
				.find( '.ultimate-border-style-selector option:selected' )
				.val();
		...
		t.find( '.' + border + ' .ultimate-border-input' ).each( function (
			index,
			elm
		) {
			const unit =
				t.find( '.ult-unit-border-width option:selected' ).val() ||
				t.find( '.ultimate-border-value' ).attr( 'data-unit' );
			const ival = $( elm ).val();
			if ( $.isNumeric( ival ) ) {
				if ( ival.match( /^[0-9]+$/ ) )
					var item =
		...
		t.find( '.' + radius + ' .ultimate-border-input' ).each( function (
			index,
			elm
		) {
			const unit =
				t.find( '.ult-unit-border-radius option:selected' ).val() ||
				t.find( '.ultimate-border-value' ).attr( 'data-unit' );
			const ival = $( elm ).val();
			if ( $.isNumeric( ival ) ) {
				if ( ival.match( /^[0-9]+$/ ) )
		...
		if ( typeof hxcolor !== 'undefined' || hxcolor != null ) {
			...
			const va = t.find( '.ultimate-colorpicker' ).val();
			if ( va != '' ) {
