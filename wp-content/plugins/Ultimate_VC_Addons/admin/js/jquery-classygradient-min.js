	...
	$.ClassyGradient = function ( element, options ) {
		...
		this.getCSS = function () {
			...
			let svg =
				...
				'<defs>' +
				'<linearGradient id="gradient" x1="0%" y1="0%" x2="' +
				svgX +
				'" y2="' +
				svgY +
				'">';
			let webkitCss = '-webkit-gradient(linear, left top, ' + webkitDir;
			let defCss = '';
		...
		this._setup = function () {
			...
			$pointsContainer.css(
				'width',
				this.settings.width +
					Math.round( this.settings.point / 2 + 1 ) +
					'px'
			);
			...
			$pointColor = $(
				'<div class="point-color"><div style="background-color: #00ff00"></div></div>'
			);
			$pointPosition = $( '<span class="point-position">%</span>' );
			$btnPointDelete = $( '<a href="#" class="delete"></a>' );
			$pointsInfosContent.append(
				$pointColor,
				$pointPosition,
				$btnPointDelete
			);
			...
			_canvas.bind( 'click', function ( e ) {
				let offset = _canvas.offset(),
					clickPosition = e.pageX - offset.left;
				clickPosition = Math.round(
					( clickPosition * 100 ) / self.settings.width
				);
				let defaultColor = '#000000',
					minDist = 999999999999;
				$.each( points, function ( i, el ) {
					if (
						parseInt( el[ 0 ] ) < clickPosition &&
						clickPosition - parseInt( el[ 0 ] ) < minDist
					) {
						minDist = clickPosition - parseInt( el[ 0 ] );
						defaultColor = el[ 1 ];
					} else if (
						parseInt( el[ 0 ] ) > clickPosition &&
						parseInt( el[ 0 ] ) - clickPosition < minDist
					) {
				...
				$.each( points, function ( i, el ) {
					if ( el[ 0 ] == clickPosition + '%' ) {
						self._selectPoint(
							$pointsContainer.find( '.point:eq(' + i + ')' )
						);
					}
				} );
			} );
			...
			$.each( points, function ( i, el ) {
				$pointsContainer.append(
					'<div class="point" style="background-color: ' +
						el[ 1 ] +
						'; left:' +
						( parseInt( el[ 0 ] ) * self.settings.width ) / 100 +
						'px;"></div>'
				);
			} );
			$pointsContainer
				.find( '.point' )
				.css( 'width', this.settings.point + 'px' )
				.css( 'height', this.settings.point + 'px' )
				.mouseup( function () {
		...
		this._selectPoint = function ( el ) {
			...
			$pointColor.ColorPickerSetColor(
				this._rgbToHex( color.split( ',' ) )
			);
			$pointColor
				.find( 'div' )
				.css( 'backgroundColor', this._rgbToHex( color.split( ',' ) ) );
		...
		this._renderCanvas = function () {
			...
			$element.find( '.point' ).each( function ( i, el ) {
				...
				let color = $( el )
					.css( 'backgroundColor' )
					.substr( 4, $( el ).css( 'backgroundColor' ).length - 5 );
		...
		this._renderToElement = function ( target, gradient ) {
			...
			if ( target === $pointsInfosContent ) {
				const tooltipRadius = parseInt(
					$pointsInfosContent.css( 'borderRadius' )
				);
				svg +=
					'<rect fill="url(#gradient)" height="100%" width="100%" rx="' +
					tooltipRadius +
					'" ry="' +
					tooltipRadius +
					'" />';
			...
			target.css(
				'background',
				'-moz-linear-gradient(' + defDir + ',' + defCss + ')'
			);
			target.css(
				'background',
				'-webkit-linear-gradient(' + defDir + ',' + defCss + ')'
			);
			target.css(
				'background',
				'-o-linear-gradient(' + defDir + ',' + defCss + ')'
			);
			target.css(
				'background',
				'-ms-linear-gradient(' + defDir + ',' + defCss + ')'
			);
			target.css(
				'background',
				'linear-gradient(' + defDir + ',' + defCss + ')'
			);
		};
		...
		this._renderToCanvas = function () {
			const gradient = _context.createLinearGradient(
				0,
				0,
				this.settings.width,
				0
		...
		this._getGradientFromString = function ( gradient ) {
			...
			$.each( _t, function ( i, el ) {
				let position;
				if (
					el.substr( el.indexOf( '%' ) - 3, el.indexOf( '%' ) ) ==
						'100' ||
					el.substr( el.indexOf( '%' ) - 3, el.indexOf( '%' ) ) ==
						'100%'
				) {
					...
					position = parseInt(
						el.substr( el.indexOf( '%' ) - 2, el.indexOf( '%' ) )
					);
					position += '%';
				} else {
					position = parseInt(
						el.substr( el.indexOf( '%' ) - 1, el.indexOf( '%' ) )
					);
					position += '%';
				}
				const color = el.substr( el.indexOf( '#' ), 7 );
	...
	$.fn.ClassyGradient = function ( options ) {
		return this.each( function () {
			if ( $( this ).data( 'ClassyGradient' ) === undefined ) {
				const plugin = new $.ClassyGradient( this, options );
